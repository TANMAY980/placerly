...
$(function () {
  ...
  --------------------------------------------------- */
  $(document).on("click", ".addAssetModalBtn", function (e) {
  e.preventDefault();
  ...
  Swal.fire({
    title: "Are you sure?",
    text: "Do you want to add this asset account?",
    icon: "question",
    reverseButtons: true,
    showCancelButton: true,
    ...
    if (result.isConfirmed) {
      $.ajax({
        ...
        success: function () {
          $("#addAssetModal").modal("hide");
          Swal.fire("Success", "Asset account added successfully", "success");
          table && table.ajax.reload(null, false);
        },
        error: function (xhr) {
          let msg = "Failed to add your asset account";
