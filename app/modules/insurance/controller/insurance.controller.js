...
const insuranceRepository=require('../repository/insurance.repository');
...
class Insurance{
    ...
    async addInsuranceAcoount(req,res){
      try {
        const{name,accounttype}=req.body;  
        const userId=new mongoose.Types.ObjectId(req.user.id); 
        const check=await insuranceRepository.getByField({name});
        if(check) {
          if(check.name===name && check.accounttype===accounttype && check.userId.toString()===userId.toString()){
            return res.status(409).json({status:false,message:"Insurance account already exists"});
          }   
        };  

        const plan={name,accounttype,userId};
        const debtdata=await insuranceRepository.save(plan);

        if(!debtdata) return res.status(400).json({status:false,message:"Failed To create Insurance account"});

        return res.status(201).json({Status:true,message:"Successfully created Insurance account"});
      } catch (error) {
        console.log(error);
