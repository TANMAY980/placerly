...
class User {
  ...
  async updatepasswordPage(req, res) {
    try {
      ...
      res.render("user/views/updatepassword", {
  ...
  async updatePassword(req, res) {
    try {
      ...
      if (!prevPassword || !newPassword || !confirmPassword) {
        return res.redirect(generateUrl("user.updatepassword.page"));
      }
      if (newPassword !== confirmPassword) {
        req.flash("error", "Enter new and confirm password same");
        return res.redirect(generateUrl("user.updatepassword.page"));
      }
      const checkuser = await userRepository.getById({ _id: req.user.id });
      ...
      const checkpass = await auth.check_password(
        prevPassword,
        checkuser.password
      );
      if (!checkpass) {
      ...
      const savePassword = await userRepository.updateById(
  ...
  async resetPasswordPage(req, res) {
    try {
      ...
      const checkuser = await userRepository.getByField({
  ...
  async resetPassword(req, res) {
    try {
      ...
      const checkuser = await userRepository.getById({
  ...
  async getDetailsPage(req,res){
    try {
        ...
        res.render("user/views/userdetails",{
  ...
  async uploadImage(req,res){
     try {
        ...
        const image = req.file.path;
        const checkuser = await userRepository.getByField({_id: new mongoose.Types.ObjectId(req.user.id)});
        if (!checkuser) {
            req.flash("error", "Something went wrong");
            res.clearCookie("accessToken");
            res.clearCookie("refreshToken");
        ...
        const saveImage = await userRepository.updateById({ image: image },new mongoose.Types.ObjectId(req.user.id));
        if (!saveImage) {
            req.flash("error", "Failed to Upload Profile image");
            return res.redirect(generateUrl("user.login.page"));
        }
  ...
  async deleteImage(req,res){
    try {
        const user=await userRepository.getById({_id:new mongoose.Types.ObjectId(req.user.id)});       
        if(!user || !user.image){
            req.flash("error","Profile Image not found");
            return res.redirect(generateUrl('user.home'));
        }
        const image=null;
        const save=await userRepository.updateById({image:image},new mongoose.Types.ObjectId(req.user.id));
        if(!save){
            req.flash("error","Profile Image not found");
            return res.redirect(generateUrl('user.details'));
        }
